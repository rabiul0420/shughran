...
if($report_info['is_current'] || $report_info['year'] == date('Y')) {
	...
	<button class="btn btn-primary dropdown-toggle" type="button" id="archive" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		...
		echo   ' <li>'.anchor('admin/bm'.( $branch_id ? '/'.$branch_id : '').'?type=half_yearly&year='.$i,'ষান্মাসিক '.$i).' </li>';
...
<td>পরিমাণ</td>
 ...
 <?php $icount=1; foreach($sources as $source) if($source->type==1){?>
...
<td class="type_1">
<?php 
  $row_info = record_row($bm_summary,'source_id',$source->id);
 $amount = $row_info['amount'];
?> 


<?php if($source->id !=32) { ?>
<a href="#"  class="editable editable-click"   data-type="number" data-table="bm_record" data-pk="<?php echo $row_info['id'];?>" data-url="<?php echo admin_url('others/detailupdate');?>" data-name="amount" data-title="Enter"><?php echo $row_info['amount'];?></a> 
<?php 
} 

else echo $row_info['amount']; 
...
<td>পরিমাণ</td>
 ...
 <?php $icount=1; foreach($sources as $source) if($source->type==2   ) {?>
