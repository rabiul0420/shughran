...
if($report_info['is_current'] || $report_info['year'] == date('Y')) {
	if($report_info['type']=='annual'){
		echo anchor('admin/organization'.( $branch_id ? '/'.$branch_id : '').('?type=half_yearly&year='.$report_info['year']),'ষান্মাসিক '.$report_info['year']); 
		echo  "&nbsp;|&nbsp;".anchor('admin/organization'.( $branch_id ? '/'.$branch_id : ''),'জুলাই-নভেম্বর\''.$report_info['year']); 
		echo "&nbsp;|&nbsp;";   echo anchor('admin/organization'.( $branch_id ? '/'.$branch_id : '').'?type=annual&year='.$report_info['year'],'বার্ষিক '.$report_info['year']);
	}
	else{
		 echo anchor('admin/organization'.( $branch_id ? '/'.$branch_id : ''),'ষান্মাসিক '.$report_info['year']); 
	...
	<button class="btn btn-primary dropdown-toggle" type="button" id="archive" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		...
		echo   ' <li>'.anchor('admin/organization'.( $branch_id ? '/'.$branch_id : '').'?type=half_yearly&year='.$i,'ষান্মাসিক '.$i).' </li>';
...
<td class="type_23"><?php   $worker_info =  sum_org_worker($org_summary,'worker',$institution->id); 
 $worker = isset($worker_info['sum']) ? $worker_info['sum'] : 0;
 $update_id = isset($worker_info['id']) ? $worker_info['id'] : 0;
?>
<a href="#"  class="editable editable-click"   data-type="number" data-table="organization_record" data-pk="<?php echo  $update_id;?>" data-url="<?php echo admin_url('organization/detailupdate');?>" data-name="worker" data-title="Enter"><?php echo $worker;?></a></td>
