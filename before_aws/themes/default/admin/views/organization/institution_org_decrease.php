...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('organization/getItitutionOrganizationDecreasedList'.($branch_id ? '/'.$branch_id : '').( $this->input->get('type') ? '?type='.$this->input->get('type') : '' ).( $this->input->get('year') ? '&year='.$this->input->get('year') : '' )  ) ?>',
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= 'সংগঠন Ghatti তালিকা ' . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
...
if($report_info['is_current'] || $report_info['year'] == date('Y')) {
    if($report_info['type']=='annual'){
        echo anchor('admin/organization/institution_org_decrease'.( $branch_id ? '/'.$branch_id : '').( '?' ).('type=half_yearly&year='.$report_info['year']),'ষান্মাসিক '.$report_info['year']); 
        echo  "&nbsp;|&nbsp;".anchor('admin/organization/institution_org_decrease'.( $branch_id ? '/'.$branch_id : ''),'জুলাই-নভেম্বর\''.$report_info['year']); 
        echo "&nbsp;|&nbsp;";   echo anchor('admin/organization/institution_org_decrease'.( $branch_id ? '/'.$branch_id : '').( '?' ).'type=annual&year='.$report_info['year'],'বার্ষিক '.$report_info['year']);
    }
    else{
         echo anchor('admin/organization/institution_org_decrease'.( $branch_id ? '/'.$branch_id : ''),'ষান্মাসিক '.$report_info['year']); 
    ...
    <button class="btn btn-primary dropdown-toggle" type="button" id="archive" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        echo   ' <li>'.anchor('admin/organization/institution_org_decrease'.( $branch_id ? '/'.$branch_id : '').'?type=half_yearly&year='.$i,'ষান্মাসিক '.$i).' </li>';
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
			 ...
			 <?php if($branch_id) {?>
                        ...
                        <li>
                            <a    href="<?= admin_url('organization/organizationdecreaseexport/'.($branch_id ? '/'.$branch_id : '').( $this->input->get('type') ?  '?type='.$this->input->get('type') : '')) ?>" id="excel_export" data-action="export_excel">
                ...
                <?php if (!empty($branches)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= admin_url('organization/institution_org_decrease') ?>"><i class="fa fa-building-o"></i> <?= 'সকল শাখা' ?></a></li>
                            <li class="divider"></li>
                            <?php
