...
if($branch_id) {
	if( ($report_type == 'annual' && $reportinfo['last_half']) || $report_type == 'half_yearly') 
	   ...
	   $result =  $this->site->query_binding("SELECT SUM(total_reader)  as total_reader, SUM(id) id from sma_highersyllabusinfo_record WHERE  branch_id = ? AND date BETWEEN ? AND ? ", array($branch_id,$start_date,$end_date));
