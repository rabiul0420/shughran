...
from sma_manpower_record WHERE  branch_id = ? AND date BETWEEN ? AND ? ", array($branch_id, $start_date, $end_date));
    ...
    function getMember($branch_id = NULL)
    {
       ...
       <ul class="dropdown-menu pull-right" role="menu">';
        ...
        if ($branch_id) {
            $this->datatables
                ->select($this->db->dbprefix('manpower') . ".id as manpowerid,  membercode,{$this->db->dbprefix('manpower')}.name, {$this->db->dbprefix('branches')}.name as branch_name, {$this->db->dbprefix('manpower')}.member_oath_date as oath_date,sessionyear,  {$this->db->dbprefix('responsibilities')}.responsibility as responsibility,thana_code, v2_v3_district_upazila({$this->db->dbprefix('manpower')}.upazila) as upazila_name", FALSE)
                ->from('member');
