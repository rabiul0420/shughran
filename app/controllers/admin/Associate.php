...
class Associate extends MY_Controller
{
    ...
    function getAssociate($branch_id = NULL)
    {
        ...
        if ($branch_id) {
            ...
            $this->datatables
                ->select($this->db->dbprefix('manpower') . ".id as manpowerid,  associatecode,   {$this->db->dbprefix('manpower')}.name, {$this->db->dbprefix('branches')}.name as branch_name, {$this->db->dbprefix('manpower')}.associate_oath_date as oath_date,sessionyear,  {$this->db->dbprefix('responsibilities')}.responsibility as responsibility,thana_code, v2_v3_district_upazila({$this->db->dbprefix('manpower')}.upazila) as upazila_name", FALSE)
                ->from('associate');
