...
class Organization extends MY_Controller
{
    ...
    function indexOLD($branch_id = NULL)
    {
       ...
       if ($this->input->get('type') && ($this->input->get('type') == 'annual'))
            $this->data['org_summary_sma'] = $this->getorg_summary_prev('annual','2020', $branch_id);	 
...
function getEntryInfo($institutions, $branch_id = NULL)
    ...
    $organization_recordinfo = $this->site->getOneRecord('organization_record','*',array('branch_id'=>$branch_id,'date <= '=>$end,'date >= '=>$start),'id desc',1,0);	
    



	return array(
	'organization_recordinfo'=>$organization_recordinfo
	);
   ...
   function detailupdate()
	 ...
	 function institutionexport($branch_id = NULL)
    {
	 ...
	 if ($branch_id) {
        ...
        $this->db
                ->select($this->db->dbprefix('institution_without_org') . ".id as id,  institution_name,   {$this->db->dbprefix('institution')}.institution_type, {$this->db->dbprefix('branches')}.name as branch_name,notes", FALSE)
				->from('institution_without_org');
		$this->db->join('institution', 'institution_without_org.institution_type_id=institution.id', 'left'); 	
        $this->db->join('branches', 'branches.id=institution_without_org.branch_id', 'left')
                ->where('branches.id', $branch_id); 
 ...
 //$this->sma->print_arrays($data);
            if (  !empty($data)) {
                ...
                $this->load->library('excel');
                $this->excel->setActiveSheetIndex(0);
                $this->excel->getActiveSheet()->setTitle('Institution list not have org');
                ...
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->institution_name);
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->institution_type);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->branch_name);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, strip_tags($data_row->notes)); 
