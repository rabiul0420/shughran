    ...
    function detailupdate()
    {
        ...
        if ($is_changeable) {
            if ($this->input->get_post('pk') && $this->input->get_post('pk') > 0) {
                ...
                $insData = array('user_id' => $this->session->userdata('user_id'), 'report_year' => date('Y'), 'branch_id' => $this->input->get_post('branch_id'), 'report_type' => $report_type['type'], $this->input->get_post('name') => $this->input->get_post('value'), 'date' => date('Y-m-d'));
                
                if($this->input->get_post('child_type_id'))
                $insData['institution_type_id'] = $this->input->get_post('child_type_id');
                
                $this->site->insertData($this->input->get_post('table'), $insData);
                $flag = 3;  //new entry
