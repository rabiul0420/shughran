...
class Poriklpona extends MY_Controller
{
    ...
    {  
		...
		$this->db->where('date between "' . $report_type['start'] . '" and "' . $report_type['end'] . '"');
			$this->data['total_planning_shakha_plan'] = $this->db->get('planning_shakha_plan')->first_row('array');

			if ($branch_id)
			$this->db->where('branch_id', $branch_id);
		$this->db->where('date between "' . $report_type['start'] . '" and "' . $report_type['end'] . '"');
			$this->data['total_planning_shakha_plan_row'] = $this->db->get('planning_shakha_plan')->num_rows();
			...
			$this->db->where('branch_id', $branch_id);
		$this->db->where('date between "' . $report_type['start'] . '" and "' . $report_type['end'] . '"');
			$this->data['total_planning_jonoshokti_plan'] = $this->db->get('planning_jonoshokti_plan')->first_row('array');
			...
			$this->db->where('branch_id', $branch_id);
		$this->db->where('date between "' . $report_type['start'] . '" and "' . $report_type['end'] . '"');
			$this->data['total_planning_training_program'] = $this->db->get('planning_training_program')->first_row('array');
