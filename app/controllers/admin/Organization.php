...
$this->data['org_summary'] = $this->getorg_summary($report_type['type'],$report_type['start'],$report_type['end'], $branch_id,$report_type);
  ...
  function getorg_summary($report_type,$start_date, $end_date,$branch_id = NULL,$reportinfo = null){
    ...
    if($branch_id) 
    $increase_outputinfo = $this->site->query_binding('SELECT *from sma_organization_record  where branch_id = ? AND date BETWEEN ? AND ?',array($branch_id,$start_date,$end_date));
    else
    $increase_outputinfo = $this->site->query_binding('SELECT *from sma_organization_record   where   date BETWEEN ? AND ?',array( $start_date,$end_date));
