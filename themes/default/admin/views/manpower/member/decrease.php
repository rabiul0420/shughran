...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
			...
			'sAjaxSource': '<?= admin_url('manpower/getDecreaseMember/'.$process->id.($branch_id ? '/'.$branch_id : '').( $this->input->get('type') ? '?type='.$this->input->get('type') : '' ).( $this->input->get('year') ? '&year='.$this->input->get('year') : '' ) ) ?>',
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= 'সদস্য ঘাটতি: '.( $process->id == 15 ? 'স্থানান্তর'  : $process->process )  . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
...
if($report_info['is_current'] || $report_info['year'] == date('Y')) {
    if($report_info['type']=='annual'){
        ...
        echo  "&nbsp;|&nbsp;".anchor('admin/manpower/memberdecreaselist/'.$process->id.( $branch_id ? '?branch_id='.$branch_id : ''),'জুলাই-নভেম্বর\''.$report_info['year']); 
        echo "&nbsp;|&nbsp;";   echo anchor('admin/manpower/memberdecreaselist/'.$process->id.( $branch_id ? '?branch_id='.$branch_id : '').($branch_id ? '&' : '?' ).'type=annual&year='.$report_info['year'],'বার্ষিক '.$report_info['year']);
    }
    else{
    ...
    <button class="btn btn-primary dropdown-toggle" type="button" id="archive" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        echo   ' <li>'.anchor('admin/manpower/memberdecreaselist/'.$process->id.( $branch_id ? '?branch_id='.$branch_id : ''),'বর্তমান ').' </li>';
                ...
                <?php if (!empty($branches)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= admin_url('manpower/memberdecreaselist/'.$process->id) ?>"><i class="fa fa-building-o"></i> <?= 'সকল শাখা' ?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($branches as $branch) {
                                echo '<li><a href="' . admin_url('manpower/memberdecreaselist/'.$process->id.'?branch_id=' . $branch->id) . '"><i class="fa fa-building"></i>' . $branch->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
