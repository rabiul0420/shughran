...
oTable3 = $('#PRData3').dataTable({
    ...
    'sAjaxSource': '<?= admin_url('manpowertransfer/getListworkerarrival'.($branch_id ? '/'.$branch_id : '').( $this->input->get('type') ? '?type='.$this->input->get('type') : '' ).( $this->input->get('year') ? '&year='.$this->input->get('year') : '' )  ) ?>',
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= 'কর্মী  আগমন  ' . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
...
if($report_info['type']=='annual'){
 echo    anchor('admin/manpowertransfer/workerarrivallist'.( $branch_id ? '?branch_id='.$branch_id : '').($branch_id ? '&' : '?' ).'type=annual&year='.$report_info['year'],'বার্ষিক '.$report_info['year']);
}
else{
...
echo   ' <li>'.anchor('admin/manpowertransfer/workerarrivallist'.( $branch_id ? '?branch_id='.$branch_id : ''),'বর্তমান ').' </li>';
                ...
                <?php if (!empty($branches)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= admin_url('manpowertransfer/workerarrivallist') ?>"><i class="fa fa-building-o"></i> <?= 'সকল শাখা' ?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($branches as $branch) {
                                echo '<li><a href="' . admin_url('manpowertransfer/workerarrivallist/' . $branch->id) . '"><i class="fa fa-building"></i>' . $branch->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
