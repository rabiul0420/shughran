...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= ' অতিরিক্ত দাওয়াত ' . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
...
if($report_info['is_current'] || $report_info['year'] == date('Y')) {
	if($report_info['type']=='annual'){
		echo anchor('admin/dawat/extra'.( $branch_id ? '/'.$branch_id : '').('?type=half_yearly&year='.$report_info['year']),'ষান্মাসিক '.$report_info['year']); 
		echo  "&nbsp;|&nbsp;".anchor('admin/dawat/extra'.( $branch_id ? '/'.$branch_id : ''),'জুলাই-নভেম্বর\''.$report_info['year']); 
		echo "&nbsp;|&nbsp;";   echo anchor('admin/dawat/extra'.( $branch_id ? '/'.$branch_id : '').'?type=annual&year='.$report_info['year'],'বার্ষিক '.$report_info['year']);
	}
	else{
		 echo anchor('admin/dawat/extra'.( $branch_id ? '/'.$branch_id : ''),'ষান্মাসিক '.$report_info['year']); 
	...
	<button class="btn btn-primary dropdown-toggle" type="button" id="archive" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		...
		echo   ' <li>'.anchor('admin/dawat/extra'.( $branch_id ? '/'.$branch_id : '').'?type=half_yearly&year='.$i,'ষান্মাসিক '.$i).' </li>';
...
<td><?php echo $detailinfo[0]['dawat_group']; ?></td>
<td  colspan="2"><?php echo $detailinfo[0]['dawat_group_increase']; ?></td>
</tr>
<tr >
...
<td><?php echo $detailinfo[0]['muharram_number']; ?></td>
<td  colspan="2"><?php echo $detailinfo[0]['muharram_number_increase']; ?></td>
...
<td><?php echo $detailinfo[0]['relative_number']; ?></td>
<td  colspan="2"><?php echo $detailinfo[0]['relative_number_increase']; ?></td>
</tr>
</tbody>
</table>
