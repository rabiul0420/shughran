...
$('#xlsx_file').change(function(e) {
    ...
    if (v != '') {
        ...
        if (validExts.indexOf(fileExt) < 0) {
            ...
            $(this).val(''); $(this).fileinput('clear');
            $('form[data-toggle="validator"]').bootstrapValidator('updateStatus', 'xlsx_file', 'NOT_VALIDATED');
            return false;
        }
        else
            return true;
...
$(function() {
    ...
    $(document).on('focus','.fixed_date', function(t) {
        $(this).datetimepicker({format: site.dateFormats.js_sdate, fontAwesome: true, todayBtn: 1, autoclose: 1, minView: 2,  startDate: reportStartDate  , endDate: reportEndDate});
    });
    
    
$('.old_date').datetimepicker({format: site.dateFormats.js_sdate, fontAwesome: true, language: 'sma', todayBtn: 1, autoclose: 1, minView: 2 ,   endDate: '2019-12-19'});
	
	
    $(document).on('focus','.old_date', function(t) {
        $(this).datetimepicker({format: site.dateFormats.js_sdate, fontAwesome: true, todayBtn: 1, autoclose: 1, minView: 2,  endDate: '2019-12-19'});
    });
...
$(document).ready(function() {
    ...
    $('body').on('click', '.product_link td:not(:first-child, :nth-child(2), :last-child)', function() {
	 ...
	 $('body').on('click', '.manpower_link td:not(:first-child, :nth-child(2), :nth-child(9),:nth-child(10), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'manpower/modal_view/' + $(this).parent('.manpower_link').attr('id') + '/'+ $(this).parent('.manpower_link').attr('status')});
        $('#myModal').modal('show');
       });
